...
class FSCILTrainer(Trainer):
    ...
    def get_base_dataloader_meta(self):
        ...
        # CategoriesSampler在dataloader调用中一次返回一个batch的index
        sampler = CategoriesSampler(trainset.targets,  # [30000, ] base classes的所有类别索引
                                    self.args.train_episode,  # 50  相当于50个batch
                                    self.args.episode_way,  # 15
                                    self.args.episode_shot + self.args.episode_query)  # 16
    ...
    def get_new_dataloader(self, session, test_new_only=False):
        ...
        if self.args.batch_size_new == 0:
            ...
            trainloader = torch.utils.data.DataLoader(dataset=trainset,
                                                      batch_size=self.args.batch_size_new,
                                                      shuffle=True,
                                                      num_workers=self.args.num_workers,
                                                      pin_memory=True)

        if test_new_only is False:
    ...
    def get_session_classes(self, session, test_new_only=False):
        if test_new_only:
            class_list = np.arange(self.args.base_class, self.args.base_class + session * self.args.way)
        else:
    ...
    def train(self):
        ...
        _, __, testloader_base = self.get_dataloader(session=0)
        ...
        for session in range(args.start_session, args.sessions):
            ...
            if session == 0:  # load base class train img label
                ...
                _, __, testloader_new = self.get_new_dataloader(session, test_new_only=True)
                print("training session: [%d]" % session)
                self.model.load_state_dict(self.best_model_dict)

                self.model.module.mode = self.args.new_mode  # avg_cos
                self.model.eval()
                trainloader.dataset.transform = testloader.dataset.transform
                self.model.module.update_fc(trainloader, np.unique(train_set.targets), session)

                tsl, tsa = self.test(self.model, testloader, args, session)
                _, tsa_base = self.test(self.model, testloader_base, args, 0)
                _, tsa_new = self.test(self.model, testloader_new, args, session)
                ...
                print('base only test acc={:.3f}'.format(tsa_base))
                print('new only test acc={:.3f}'.format(tsa_new))
                result_list.append('Session {}, Test Best Epoch {},\nbest test Acc {:.4f}\n'.format(
                    session, self.trlog['max_acc_epoch'], self.trlog['max_acc'][session]))
    ...
    def base_train(self, model, trainloader, optimizer, scheduler, epoch, args):
        ...
        for i, batch in enumerate(tqdm_gen, 1):
            data, true_label = [_.cuda() for _ in batch]  # 一个batch是240=15 * 16张图

            k = args.episode_way * args.episode_shot  # 15 * 1 = 15（PIL base size）
            proto, query = data[:k], data[k:]  # proto: support set, query: query set
            # sample low_way data
            proto_tmp = deepcopy(proto.reshape(args.episode_shot,
                                               args.episode_way,
                                               proto.shape[1],
                                               proto.shape[2],
                                               proto.shape[3])[:args.low_shot, :args.low_way, :, :, :].flatten(0, 1))
            # proto_test = proto.reshape(args.episode_shot,
            #                            args.episode_way,
            #                            proto.shape[1],
            #                            proto.shape[2],
            #                            proto.shape[3])
            # proto_test_1 = proto_test[:args.low_shot, :args.low_way, :, :, :]
            # proto_test_2 = proto_test_1.flatten(0, 1)
            # proto_tmp = deepcopy(proto)
            query_tmp = deepcopy(query.reshape(args.episode_query,
                                               args.episode_way,
                                               query.shape[1],
                                               query.shape[2],
                                               query.shape[3])[:, :args.low_way, :, :, :].flatten(0, 1))
            # query_test = query.reshape(args.episode_query,
            #                            args.episode_way,
            #                            query.shape[1],
            #                            query.shape[2],
            #                            query.shape[3])
            # query_test_1 = query_test[:, :args.low_way, :, :, :]
            # query_test_2 = query_test_1.flatten(0, 1)
            # query_tmp = deepcopy(query)
            # random choose rotate degree 随机旋转
            proto_tmp, query_tmp = self.replace_to_rotate(proto_tmp, query_tmp)  # pseudo incremental classes

            model.module.mode = 'encoder'  # 通过resnet
            data = model(data)  # base support and query set  [240, 64]
            proto_tmp = model(proto_tmp)  # pseudo incremental learning support set  [15, 64]
            query_tmp = model(query_tmp)  # pseudo incremental learning query set  [225, 64]
            # 经过model的输出实际得到的就相当于分类器权重
            # k = args.episode_way * args.episode_shot = 15
            proto, query = data[:k], data[k:]  # base support and query set

            proto = proto.view(args.episode_shot, args.episode_way, proto.shape[-1])  # [1, 15, 64]
            query = query.view(args.episode_query, args.episode_way, query.shape[-1])  # [15, 15, 64]

            proto_tmp = proto_tmp.view(args.low_shot, args.low_way, proto.shape[-1])  # [1, 15, 64]
            query_tmp = query_tmp.view(args.episode_query, args.low_way, query.shape[-1])  # [15, 15, 64]

            proto = proto.mean(0).unsqueeze(0)
            proto_tmp = proto_tmp.mean(0).unsqueeze(0)

            # 将pseudo base weight 和 pseudo incremental weight结合
            proto = torch.cat([proto, proto_tmp], dim=1)  # [1, 30, 64]
            query = torch.cat([query, query_tmp], dim=1)  # [15, 30, 64]

            proto = proto.unsqueeze(0)  # [1, 1, 30, 64]
            query = query.unsqueeze(0)  # [1, 15, 30, 64]
    ...
    def test(self, model, testloader, args, session):
        test_class = args.base_class + session * args.way  # 用基本类加上所有增量类进行测试
        model = model.eval()
        vl = Averager()
        va = Averager()
        with torch.no_grad():
            for i, batch in enumerate(testloader, 1):
                ...
                test_label = test_label.type(torch.long).to(self.args.device)

                model.module.mode = 'encoder'
                query = model(data)  # [100, 64]
                query = query.unsqueeze(0).unsqueeze(0)  # [100, 64] -> [1, 1, 100, 64]

                proto = model.module.fc.weight[:test_class, :].detach()  # [num_classes, 64]
                proto = proto.unsqueeze(0).unsqueeze(0)  # [num_classes, 64] -> [1, 1, num_classes, 64]
